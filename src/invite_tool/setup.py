...
confDefault = {
    "authentik": {"url": "", "key": ""},
    "twingate": {"use": False, "network": "", "key": ""},
}


def createConfFile():
    with open("./conf/conf.yml", "w+") as f:
        yaml.safe_dump(confDefault, f)
...
def headless(data: dict, ignore=False):
    ...
    if getExisting() != confDefault and not ignore:
...
def interactive(ignore=False):
    ...
    if getExisting() != confDefault and not ignore:
        print(
            "It looks like Invite Tool has already been configured. If you would like to ignore and override this preexisting configuration, rerun this program again with the `-i` flag."
        )
        exit(1)
    ...
    if ignore:
        print(
            "INFO: Ignore flag is present. Any preexisting configuration will be overwritten."
        )

    print(
        "NOTE: When asked to input a URL, only input the domain name portion. Do not include the protocol or path."
    )
    conf = confDefault.copy()
    ...
    if conf["twingate"]["use"].lower() == "y" or conf["twingate"]["use"] == "":
        ...
        conf["twingate"]["network"] = input(
            " Network name (just the subdomain of twingate.com): "
        )
...
def cli():
    ...
    parser = argparse.ArgumentParser(
        description="Setup program for Invite Tool to configure Authentik and Twingate connections."
    )
    ...
    parser.add_argument(
        "--headless",
        type=json.loads,
        help="Headless mode; requires JSON data enclosed in a pair of single quotes.",
    )
    parser.add_argument(
        "-i",
        action="store_true",
        default="store_false",
        help="Ignore and override any existing configuration.",
    )
...
if __name__ == "__main__":
    ...
    subprocess.Popen(
        "pip install -r requires.txt".split(" "),
